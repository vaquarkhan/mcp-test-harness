...
| Defaults | Safe; `quality_gate` / `manifest_gate` / `agents_md_gate` are **opt-in** |
...
| Security | payload packs (injection, path, leak, Suite A egress/covert, exec capability reduction), agents_md_gate / scan-agents (hidden Unicode in AGENTS.md), auth boundaries, SARIF, quality_gate, OWASP rules � Pages tab: [security.html](https://vaquarkhan.github.io/mcp-test-harness/guide/security.html) |
...
| Moat (Tier 1) | manifest rug-pull gate · agents_md hidden-Unicode scan � history/trends (next) � flakiness � version matrix |
...
2. **Coding agents:** [llms.txt](../llms.txt) � [llms-full.txt](../llms-full.txt) � skill [`skills/mcp-test-harness`](../skills/mcp-test-harness/SKILL.md) � paste [templates/AGENTS.md](templates/AGENTS.md) into consumer repos  
3. **Security reviewer:** Part 2 ? [SECURITY_TESTING](SECURITY_TESTING.md) ? enable `quality_gate` + `manifest_gate` + `agents_md_gate` / `mcp-test scan-agents` ? SARIF in Code Scanning ? [Bastion handbook](https://vaquarkhan.github.io/MCP-Bastion/guide/bible.html) for runtime  
4. **Platform / perf:** [PERFORMANCE](PERFORMANCE.md) ? [Load tab](https://vaquarkhan.github.io/mcp-test-harness/guide/performance.html) ? RFC-006 stateless  
5. **Resiliency:** [Chaos tab](https://vaquarkhan.github.io/mcp-test-harness/guide/chaos.html) ? [Resiliency tab](https://vaquarkhan.github.io/mcp-test-harness/guide/resiliency.html) ? RFC-005  
6. **Compliance:** [CRA_COMPLIANCE](CRA_COMPLIANCE.md) ? [ENTERPRISE_GOVERNANCE](ENTERPRISE_GOVERNANCE.md) ? evidence packs  
...
MCP Test Harness 5.2.0 � MIT License � Author: Vaquar Khan  
