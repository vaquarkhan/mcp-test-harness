...
**PyPI / keywords / Project links:** keep [project] `keywords` and `classifiers` in [pyproject.toml](../pyproject.toml) current. [project.urls](https://packaging.python.org/en/latest/specifications/declaring-project-metadata/#urls) can include `Docker`, `Architecture`, `Editors`, and `Collections` ([docs/COLLECTIONS.md](COLLECTIONS.md) — Postman-style multi-step flows doc) for PyPI’s **Project links** sidebar. The optional [mcplint] extra is a separate **install path** and should be mentioned in README when relevant.
