...
[![PyPI version](https://img.shields.io/badge/pypi-v5.0.0-blue)](https://pypi.org/project/mcp-test-harness/)
...
**Current release: [5.0.0](https://pypi.org/project/mcp-test-harness/)** · Docker `5.0.0` · [CHANGELOG](CHANGELOG.md)

| Go to | Link |
|-------|------|
| **Documentation handbook** | https://vaquarkhan.github.io/mcp-test-harness/guide/handbook.html |
| **Docs tabs** (Load · Chaos · Resiliency · Security · Reports) | https://vaquarkhan.github.io/mcp-test-harness/guide/ |
...
**Latest Release** - install from **[PyPI](https://pypi.org/project/mcp-test-harness/)** (`pip install mcp-test-harness`) or use the **OCI image** on **GitHub Container Registry:** **`ghcr.io/vaquarkhan/mcp-test-harness`**. Tags: **`latest`** and **`5.0.0`** (runtime, `mcp-test` entrypoint), **`dev`** and **`5.0.0-dev`** (pytest + dev extras). [Browse tags on GHCR](https://github.com/vaquarkhan/mcp-test-harness/pkgs/container/mcp-test-harness/versions) · `docker run --rm ghcr.io/vaquarkhan/mcp-test-harness:latest --version` · [docs/DOCKER.md](docs/DOCKER.md) · [docs/RELEASING.md](docs/RELEASING.md)
...
| Performance | `assert_latency` / `assert_throughput` / `assert_load_phases` / `assert_stateless_throughput`, baselines, SLO params |
...
| **Platform QA** | Tool **coverage map**, **unified portal** in HTML/JSON, **security payload** packs, **resiliency** assertions, **performance baselines**, **`assert_throughput`** / **`assert_load_phases`** / **`assert_stateless_throughput`** SLO params | [docs/POSITIONING.md](docs/POSITIONING.md) · [docs/SECURITY_TESTING.md](docs/SECURITY_TESTING.md) · [RFC-006](docs/design/RFC-006-stateless-mcp.md) |
...
|---|----------------|--------------|
| **1** | **Tri-modal CI architecture** | Functional + regression + performance in one `mcp-test` run - `assert_latency` (p90/p95/p99 + warmup), `assert_throughput` / `assert_load_phases` (`duration_s`, `min_rps`, `max_p90/p95/p99_ms`, `max_error_rate`, weighted mixes), `assert_tool_idempotent`, JSON **performance baselines** |
...
- **Performance:** `assert_latency`, `assert_throughput`, `assert_load_phases`, `assert_latency_within_baseline`
...
| Document | Contents |
|----------|----------|
| [docs/HANDBOOK.md](docs/HANDBOOK.md) | **Documentation handbook** - security-first rationale, CI scan report, feature map, end references |
...
| MCP assertions | `assert_tool_call`, `assert_resource_read`, `assert_prompt`, `assert_capabilities`, `assert_snapshot`, plus `assert_tool_schema`, `assert_protocol_version`, `assert_tool_idempotent`, **`assert_latency`** (p90/p95/p99/mean + **warmup**), **`assert_throughput`** / **`assert_load_phases`** (concurrent load + **`duration_s`**, **`min_rps`**, **`max_p90/p95/p99_ms`**, **`max_error_rate`**, weighted **`calls`**), **`assert_latency_within_baseline`**, **security payload packs** - see [docs/PERFORMANCE.md](docs/PERFORMANCE.md), [docs/SECURITY_TESTING.md](docs/SECURITY_TESTING.md) |
...
**Current stable version:** **5.0.0** (see [CHANGELOG.md](CHANGELOG.md)). Core harness (lightweight: `mcp` + YAML + anyio; **no** MCP-Bastion / Presidio stack):

```bash
pip install mcp-test-harness
# pin, if you need a fixed version:
# pip install mcp-test-harness==5.0.0
```

**Same release as a container (GHCR, no local Python):** `docker pull ghcr.io/vaquarkhan/mcp-test-harness:5.0.0` or `:latest` - see the [image](#docker) section for `docker run` and dev tags.
...
Pre-built **runtime** and **dev** (test tooling) images are defined in the repo [`Dockerfile`](Dockerfile) (and [`.dockerignore`](.dockerignore) keeps the build context small). Each **`v*`** git tag triggers CI that pushes **`ghcr.io/vaquarkhan/mcp-test-harness`**: e.g. **`:5.0.0`**, **`:latest`**, **`:5.0.0-dev`**, **`:dev`**. Quick pull: `docker pull ghcr.io/vaquarkhan/mcp-test-harness:latest`. [All versions (GHCR)](https://github.com/vaquarkhan/mcp-test-harness/pkgs/container/mcp-test-harness/versions) · [docs/RELEASING.md](docs/RELEASING.md) · [docs/DOCKER.md](docs/DOCKER.md).
  ...
  test:
    ...
    steps:
      ...
      - name: Test MCP Server
        id: mcp
        uses: vaquarkhan/mcp-test-harness@v5.0.0
        with:
...
Marketplace: [mcp-test-harness](https://github.com/marketplace/actions/mcp-test-harness). Path form: `vaquarkhan/mcp-test-harness/.github/actions/mcp-test@v5.0.0` - see [examples/example_github_actions.md](examples/example_github_actions.md).
...
MCP Test Harness provides framework-specific testing helpers. Each package auto-installs `mcp-test-harness` as a dependency. **Version** and **download** columns use live badges from [ecosystem-downloads.json](html/assets/ecosystem-downloads.json) (**all-time total** via PePy when indexed; **on PyPI** until new packages are indexed). **Trends:** [pypistats.org](https://pypistats.org/).

**One-place totals:** [Integrations download dashboard](https://vaquarkhan.github.io/mcp-test-harness/integrations.html) · [ecosystem-downloads.json](html/assets/ecosystem-downloads.json) (sum of all 26 packages, refreshed daily) · totals update after merge · updated 2026-08-08.

| Package | Tests for | Version | Downloads |
|---------|-----------|---------|-----------|
| `mcp-test-harness` | Core CLI + assertions | [![PyPI](https://img.shields.io/pypi/v/mcp-test-harness?label=pypi)](https://pypi.org/project/mcp-test-harness/) | [![Downloads](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Fvaquarkhan%2Fmcp-test-harness%2Fmain%2Fhtml%2Fassets%2Fbadges%2Fmcp-test-harness-downloads.json)](https://pypistats.org/packages/mcp-test-harness) · [pypistats](https://pypistats.org/packages/mcp-test-harness) |
| `mcp-test-harness-fastmcp` | FastMCP servers | [![PyPI](https://img.shields.io/pypi/v/mcp-test-harness-fastmcp?label=pypi)](https://pypi.org/project/mcp-test-harness-fastmcp/) | [![Downloads](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Fvaquarkhan%2Fmcp-test-harness%2Fmain%2Fhtml%2Fassets%2Fbadges%2Fmcp-test-harness-fastmcp-downloads.json)](https://pypistats.org/packages/mcp-test-harness-fastmcp) · [pypistats](https://pypistats.org/packages/mcp-test-harness-fastmcp) |
| `mcp-test-harness-openai` | OpenAI function calling | [![PyPI](https://img.shields.io/pypi/v/mcp-test-harness-openai?label=pypi)](https://pypi.org/project/mcp-test-harness-openai/) | [![Downloads](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Fvaquarkhan%2Fmcp-test-harness%2Fmain%2Fhtml%2Fassets%2Fbadges%2Fmcp-test-harness-openai-downloads.json)](https://pypistats.org/packages/mcp-test-harness-openai) · [pypistats](https://pypistats.org/packages/mcp-test-harness-openai) |
| `mcp-test-harness-anthropic` | Anthropic Claude tool use | [![PyPI](https://img.shields.io/pypi/v/mcp-test-harness-anthropic?label=pypi)](https://pypi.org/project/mcp-test-harness-anthropic/) | [![Downloads](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Fvaquarkhan%2Fmcp-test-harness%2Fmain%2Fhtml%2Fassets%2Fbadges%2Fmcp-test-harness-anthropic-downloads.json)](https://pypistats.org/packages/mcp-test-harness-anthropic) · [pypistats](https://pypistats.org/packages/mcp-test-harness-anthropic) |
| `mcp-test-harness-bedrock` | AWS Bedrock agents | [![PyPI](https://img.shields.io/pypi/v/mcp-test-harness-bedrock?label=pypi)](https://pypi.org/project/mcp-test-harness-bedrock/) | [![Downloads](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Fvaquarkhan%2Fmcp-test-harness%2Fmain%2Fhtml%2Fassets%2Fbadges%2Fmcp-test-harness-bedrock-downloads.json)](https://pypistats.org/packages/mcp-test-harness-bedrock) · [pypistats](https://pypistats.org/packages/mcp-test-harness-bedrock) |
| `mcp-test-harness-gemini` | Google Gemini | [![PyPI](https://img.shields.io/pypi/v/mcp-test-harness-gemini?label=pypi)](https://pypi.org/project/mcp-test-harness-gemini/) | [![Downloads](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Fvaquarkhan%2Fmcp-test-harness%2Fmain%2Fhtml%2Fassets%2Fbadges%2Fmcp-test-harness-gemini-downloads.json)](https://pypistats.org/packages/mcp-test-harness-gemini) · [pypistats](https://pypistats.org/packages/mcp-test-harness-gemini) |
| `mcp-test-harness-langchain` | LangChain MCP tools | [![PyPI](https://img.shields.io/pypi/v/mcp-test-harness-langchain?label=pypi)](https://pypi.org/project/mcp-test-harness-langchain/) | [![Downloads](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Fvaquarkhan%2Fmcp-test-harness%2Fmain%2Fhtml%2Fassets%2Fbadges%2Fmcp-test-harness-langchain-downloads.json)](https://pypistats.org/packages/mcp-test-harness-langchain) · [pypistats](https://pypistats.org/packages/mcp-test-harness-langchain) |
| `mcp-test-harness-crewai` | CrewAI agents | [![PyPI](https://img.shields.io/pypi/v/mcp-test-harness-crewai?label=pypi)](https://pypi.org/project/mcp-test-harness-crewai/) | [![Downloads](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Fvaquarkhan%2Fmcp-test-harness%2Fmain%2Fhtml%2Fassets%2Fbadges%2Fmcp-test-harness-crewai-downloads.json)](https://pypistats.org/packages/mcp-test-harness-crewai) · [pypistats](https://pypistats.org/packages/mcp-test-harness-crewai) |
| `mcp-test-harness-llamaindex` | LlamaIndex tools | [![PyPI](https://img.shields.io/pypi/v/mcp-test-harness-llamaindex?label=pypi)](https://pypi.org/project/mcp-test-harness-llamaindex/) | [![Downloads](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Fvaquarkhan%2Fmcp-test-harness%2Fmain%2Fhtml%2Fassets%2Fbadges%2Fmcp-test-harness-llamaindex-downloads.json)](https://pypistats.org/packages/mcp-test-harness-llamaindex) · [pypistats](https://pypistats.org/packages/mcp-test-harness-llamaindex) |
| `mcp-test-harness-groq` | Groq inference | [![PyPI](https://img.shields.io/pypi/v/mcp-test-harness-groq?label=pypi)](https://pypi.org/project/mcp-test-harness-groq/) | [![Downloads](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Fvaquarkhan%2Fmcp-test-harness%2Fmain%2Fhtml%2Fassets%2Fbadges%2Fmcp-test-harness-groq-downloads.json)](https://pypistats.org/packages/mcp-test-harness-groq) · [pypistats](https://pypistats.org/packages/mcp-test-harness-groq) |
| `mcp-test-harness-mistral` | Mistral AI | [![PyPI](https://img.shields.io/pypi/v/mcp-test-harness-mistral?label=pypi)](https://pypi.org/project/mcp-test-harness-mistral/) | [![Downloads](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Fvaquarkhan%2Fmcp-test-harness%2Fmain%2Fhtml%2Fassets%2Fbadges%2Fmcp-test-harness-mistral-downloads.json)](https://pypistats.org/packages/mcp-test-harness-mistral) · [pypistats](https://pypistats.org/packages/mcp-test-harness-mistral) |
| `mcp-test-harness-cohere` | Cohere | [![PyPI](https://img.shields.io/pypi/v/mcp-test-harness-cohere?label=pypi)](https://pypi.org/project/mcp-test-harness-cohere/) | [![Downloads](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Fvaquarkhan%2Fmcp-test-harness%2Fmain%2Fhtml%2Fassets%2Fbadges%2Fmcp-test-harness-cohere-downloads.json)](https://pypistats.org/packages/mcp-test-harness-cohere) · [pypistats](https://pypistats.org/packages/mcp-test-harness-cohere) |
| `mcp-test-harness-azure` | Azure OpenAI | [![PyPI](https://img.shields.io/pypi/v/mcp-test-harness-azure?label=pypi)](https://pypi.org/project/mcp-test-harness-azure/) | [![Downloads](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Fvaquarkhan%2Fmcp-test-harness%2Fmain%2Fhtml%2Fassets%2Fbadges%2Fmcp-test-harness-azure-downloads.json)](https://pypistats.org/packages/mcp-test-harness-azure) · [pypistats](https://pypistats.org/packages/mcp-test-harness-azure) |
| `mcp-test-harness-vertexai` | Google Vertex AI | [![PyPI](https://img.shields.io/pypi/v/mcp-test-harness-vertexai?label=pypi)](https://pypi.org/project/mcp-test-harness-vertexai/) | [![Downloads](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Fvaquarkhan%2Fmcp-test-harness%2Fmain%2Fhtml%2Fassets%2Fbadges%2Fmcp-test-harness-vertexai-downloads.json)](https://pypistats.org/packages/mcp-test-harness-vertexai) · [pypistats](https://pypistats.org/packages/mcp-test-harness-vertexai) |
| `mcp-test-harness-huggingface` | Hugging Face Inference | [![PyPI](https://img.shields.io/pypi/v/mcp-test-harness-huggingface?label=pypi)](https://pypi.org/project/mcp-test-harness-huggingface/) | [![Downloads](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Fvaquarkhan%2Fmcp-test-harness%2Fmain%2Fhtml%2Fassets%2Fbadges%2Fmcp-test-harness-huggingface-downloads.json)](https://pypistats.org/packages/mcp-test-harness-huggingface) · [pypistats](https://pypistats.org/packages/mcp-test-harness-huggingface) |
| `mcp-test-harness-deepseek` | DeepSeek AI | [![PyPI](https://img.shields.io/pypi/v/mcp-test-harness-deepseek?label=pypi)](https://pypi.org/project/mcp-test-harness-deepseek/) | [![Downloads](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Fvaquarkhan%2Fmcp-test-harness%2Fmain%2Fhtml%2Fassets%2Fbadges%2Fmcp-test-harness-deepseek-downloads.json)](https://pypistats.org/packages/mcp-test-harness-deepseek) · [pypistats](https://pypistats.org/packages/mcp-test-harness-deepseek) |
| `mcp-test-harness-together` | Together AI | [![PyPI](https://img.shields.io/pypi/v/mcp-test-harness-together?label=pypi)](https://pypi.org/project/mcp-test-harness-together/) | [![Downloads](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Fvaquarkhan%2Fmcp-test-harness%2Fmain%2Fhtml%2Fassets%2Fbadges%2Fmcp-test-harness-together-downloads.json)](https://pypistats.org/packages/mcp-test-harness-together) · [pypistats](https://pypistats.org/packages/mcp-test-harness-together) |
| `mcp-test-harness-fireworks` | Fireworks AI | [![PyPI](https://img.shields.io/pypi/v/mcp-test-harness-fireworks?label=pypi)](https://pypi.org/project/mcp-test-harness-fireworks/) | [![Downloads](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Fvaquarkhan%2Fmcp-test-harness%2Fmain%2Fhtml%2Fassets%2Fbadges%2Fmcp-test-harness-fireworks-downloads.json)](https://pypistats.org/packages/mcp-test-harness-fireworks) · [pypistats](https://pypistats.org/packages/mcp-test-harness-fireworks) |
| `mcp-test-harness-ollama` | Ollama local models | [![PyPI](https://img.shields.io/pypi/v/mcp-test-harness-ollama?label=pypi)](https://pypi.org/project/mcp-test-harness-ollama/) | [![Downloads](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Fvaquarkhan%2Fmcp-test-harness%2Fmain%2Fhtml%2Fassets%2Fbadges%2Fmcp-test-harness-ollama-downloads.json)](https://pypistats.org/packages/mcp-test-harness-ollama) · [pypistats](https://pypistats.org/packages/mcp-test-harness-ollama) |
| `mcp-test-harness-openrouter` | OpenRouter gateway | [![PyPI](https://img.shields.io/pypi/v/mcp-test-harness-openrouter?label=pypi)](https://pypi.org/project/mcp-test-harness-openrouter/) | [![Downloads](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Fvaquarkhan%2Fmcp-test-harness%2Fmain%2Fhtml%2Fassets%2Fbadges%2Fmcp-test-harness-openrouter-downloads.json)](https://pypistats.org/packages/mcp-test-harness-openrouter) · [pypistats](https://pypistats.org/packages/mcp-test-harness-openrouter) |
| `mcp-test-harness-litellm` | LiteLLM proxy | [![PyPI](https://img.shields.io/pypi/v/mcp-test-harness-litellm?label=pypi)](https://pypi.org/project/mcp-test-harness-litellm/) | [![Downloads](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Fvaquarkhan%2Fmcp-test-harness%2Fmain%2Fhtml%2Fassets%2Fbadges%2Fmcp-test-harness-litellm-downloads.json)](https://pypistats.org/packages/mcp-test-harness-litellm) · [pypistats](https://pypistats.org/packages/mcp-test-harness-litellm) |
| `mcp-test-harness-xai` | xAI Grok | [![PyPI](https://img.shields.io/pypi/v/mcp-test-harness-xai?label=pypi)](https://pypi.org/project/mcp-test-harness-xai/) | [![Downloads](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Fvaquarkhan%2Fmcp-test-harness%2Fmain%2Fhtml%2Fassets%2Fbadges%2Fmcp-test-harness-xai-downloads.json)](https://pypistats.org/packages/mcp-test-harness-xai) · [pypistats](https://pypistats.org/packages/mcp-test-harness-xai) |
| `mcp-test-harness-autogen` | Microsoft AutoGen | [![PyPI](https://img.shields.io/pypi/v/mcp-test-harness-autogen?label=pypi)](https://pypi.org/project/mcp-test-harness-autogen/) | [![Downloads](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Fvaquarkhan%2Fmcp-test-harness%2Fmain%2Fhtml%2Fassets%2Fbadges%2Fmcp-test-harness-autogen-downloads.json)](https://pypistats.org/packages/mcp-test-harness-autogen) · [pypistats](https://pypistats.org/packages/mcp-test-harness-autogen) |
| `mcp-test-harness-langgraph` | LangGraph stateful agents | [![PyPI](https://img.shields.io/pypi/v/mcp-test-harness-langgraph?label=pypi)](https://pypi.org/project/mcp-test-harness-langgraph/) | [![Downloads](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Fvaquarkhan%2Fmcp-test-harness%2Fmain%2Fhtml%2Fassets%2Fbadges%2Fmcp-test-harness-langgraph-downloads.json)](https://pypistats.org/packages/mcp-test-harness-langgraph) · [pypistats](https://pypistats.org/packages/mcp-test-harness-langgraph) |
| `mcp-test-harness-pydantic-ai` | Pydantic AI agents | [![PyPI](https://img.shields.io/pypi/v/mcp-test-harness-pydantic-ai?label=pypi)](https://pypi.org/project/mcp-test-harness-pydantic-ai/) | [![Downloads](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Fvaquarkhan%2Fmcp-test-harness%2Fmain%2Fhtml%2Fassets%2Fbadges%2Fmcp-test-harness-pydantic-ai-downloads.json)](https://pypistats.org/packages/mcp-test-harness-pydantic-ai) · [pypistats](https://pypistats.org/packages/mcp-test-harness-pydantic-ai) |
| `mcp-test-harness-openai-agents` | OpenAI Agents SDK | [![PyPI](https://img.shields.io/pypi/v/mcp-test-harness-openai-agents?label=pypi)](https://pypi.org/project/mcp-test-harness-openai-agents/) | [![Downloads](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Fvaquarkhan%2Fmcp-test-harness%2Fmain%2Fhtml%2Fassets%2Fbadges%2Fmcp-test-harness-openai-agents-downloads.json)](https://pypistats.org/packages/mcp-test-harness-openai-agents) · [pypistats](https://pypistats.org/packages/mcp-test-harness-openai-agents) |
