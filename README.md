...
| **llms.txt** (agent-readable) | https://vaquarkhan.github.io/mcp-test-harness/llms.txt |
...
### AGENTS.md hidden-Unicode scan -- agent rules integrity

Opt-in defensive gate: fail CI when `AGENTS.md` / `CLAUDE.md` / `.cursorrules` (and related rule/skill files) contain invisible Unicode used for instruction smuggling (Tags block, bidi overrides, zero-width). Disabled by default — **non-breaking**.
...
```python
from mcp_test_harness import assert_agents_md_clean
...
Copy-paste: [examples/example_toa_verify.md](examples/example_toa_verify.md). Pin the emitter public key with the flags documented in the toa repo when you need a specific signer.
