"""OpenAI-specific testing helpers for MCP Test Harness.

Thin wrappers around core assertions plus MCP→OpenAI schema converters.
"""
...
async def assert_openai_tool(
    session: Any,
    tool_name: str,
    arguments: dict[str, Any] | None = None,
    expected_text: str | None = None,
) -> Any:
    """Call an MCP tool (as used from OpenAI) and optionally check text."""
    args = arguments or {}
    ...
    if expected_text is not None:
        ...
        if not content:
            raise MCPAssertionError(
                f"OpenAI tool '{tool_name}' returned no content"
            )
        actual_text = getattr(content[0], "text", None)
        if actual_text != expected_text:
            raise MCPAssertionError(
                f"OpenAI tool '{tool_name}' text mismatch: "
...
def create_openai_test_config(
    ...
    *,
    ...
    """Build an mcp-test.yaml-shaped config for OpenAI MCP testing."""
    server: dict[str, Any] = {
