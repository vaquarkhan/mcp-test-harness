"""LlamaIndex-specific testing helpers for MCP Test Harness.

Thin wrappers around core assertions plus MCP→LlamaIndex schema converters.
"""
...
async def assert_llamaindex_tool(
    session: Any,
    tool_name: str,
    arguments: dict[str, Any] | None = None,
    expected_text: str | None = None,
) -> Any:
    """Call an MCP tool (as used from LlamaIndex) and optionally check text."""
    args = arguments or {}
    ...
    if expected_text is not None:
        ...
        if not content:
            raise MCPAssertionError(
                f"LlamaIndex tool '{tool_name}' returned no content"
            )
        actual_text = getattr(content[0], "text", None)
        if actual_text != expected_text:
            raise MCPAssertionError(
                f"LlamaIndex tool '{tool_name}' text mismatch: "
...
def mcp_tools_to_llamaindex_tools(tools: list[Any]) -> list[dict[str, Any]]:
...
def create_llamaindex_test_config(
    ...
    *,
    ...
    """Build an mcp-test.yaml-shaped config for LlamaIndex MCP testing."""
    server: dict[str, Any] = {
