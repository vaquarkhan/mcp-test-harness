...
```python
from mcp_test_harness_mistral import assert_mistral_tool, mcp_tools_to_mistral_functions, create_mistral_test_config

async def test_tool(mcp_server):
    await assert_mistral_tool(mcp_server, "my_tool", {}, expected_text="ok")

def test_schema_bridge(mcp_tools):
    # mcp_tools from list_tools — convert for Mistral AI SDKs
    return mcp_tools_to_mistral_functions(mcp_tools)

cfg = create_mistral_test_config("python server.py", timeout=45)
```

## Helpers

- Tool call assertion with optional text check
- MCP tool → Mistral AI schema converter
